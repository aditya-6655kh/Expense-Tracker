...
const Dashboard = () => {
  ...
  const getLocalMonthYear = (dateString) => {
    ...
    // Directly extract YYYY-MM from ISO strings (avoids timezone shift bugs)
  ...
  const availableMonths = useMemo(() => {
    const months = transactions.map(t => getLocalMonthYear(t.date)); // Get YYYY-MM format
  ...
  const filteredTransactions = useMemo(() => {
    ...
    console.log("Sample date result:", transactions[0] && getLocalMonthYear(transactions[0].date));
  ...
  const displayChartData = useMemo(() => {
    ...
    filteredTransactions.forEach(t => {
      if(t.category_type === "expense") {
        const categoryName = t.category_name || t.name || "Uncategorized";
        chartMap[categoryName] = (chartMap[categoryName] || 0) + parseFloat(t.amount);
      }
    });
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-gray-500 text-sm font-medium "> Filtered By : </span>
                <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="text-sm border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 bg-white px-3 py-1.5 outline-none cursor-pointer transition">
